...
`.github/workflows/riepilogo.yml` runs `pnpm job` on GitHub Actions. It is committed **switched off**: the job runs only when the repository variable `INTERPELS_ATTIVO` is `true`; otherwise every run, scheduled or manual, skips it.
...
To switch it on: add the secrets, then set the variable `INTERPELS_ATTIVO` to `true` (Settings → Secrets and variables → Actions).
